...
class QASample:
    """
    Parser for QA Samples from datasets like Squad.
    Can parse from json.
    """
    ...
    def __init__(
        ...
        self.sup_ids = (
            sup_ids if sup_ids is not None else [self.sup_id_from_answer_sentence()]
        )
    ...
    def sup_id_from_answer_sentence(self) -> List[List[int]]:
        ...
        if self.answer_dict["answer_start"] != -1 and self.answer_dict["text"] != "":
            return self.get_sentence_span_from_char_position(
                self.context, self.answer_dict["answer_start"]
            )
